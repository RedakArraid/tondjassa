...
async function renew(portal: Portal, oldToken: string): Promise<string | null> {
  ...
  const run = async () => {
    const current = localStorage.getItem(keys[portal]);
    ...
    if (!response.ok) {
      if (localStorage.getItem(keys[portal]) === oldToken) localStorage.removeItem(keys[portal]);
      return null;
    }
    const data = await response.json();
    localStorage.setItem(keys[portal], data.accessToken);
...
export function rememberOrderAccess(reference: string, token: string) {
  localStorage.setItem(`mm_order_access_${reference}`, token);
}
export async function apiFetch(input: RequestInfo | URL, init: RequestInit = {}): Promise<Response> {
  ...
  const staffToken = localStorage.getItem(keys.staff);
  ...
  const token = supplied || localStorage.getItem(keys[portal]);
  ...
  if (reference) {
    const orderToken = localStorage.getItem(`mm_order_access_${decodeURIComponent(reference)}`);
...
export async function logoutSession(portal: Portal) {
  const token = localStorage.getItem(keys[portal]);
  if (token) {
  ...
  if (portal === 'staff') localStorage.removeItem('admin_user');
}
